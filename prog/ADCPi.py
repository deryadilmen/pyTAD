...
class ADCPi:
    ...
    def set_bit_rate(self, rate):
        ...
        if rate == 12:
            ...
            # bit 2 = 1, bit 3 = 1
            print(self.__adc1_address,self.__adc2_address)
            try:
              if self.__adc1_address !=0:
                self.__adc1_conf = self.__updatebyte(self.__adc1_conf, 0xF3, 0x0C)
            except:
                print('error setting ADC1 bitrate')
            try:
              if self.__adc2_address !=0:
                self.__adc2_conf = self.__updatebyte(self.__adc2_conf, 0xF3, 0x0C)
            except:
                print('error setting ADC2 bitrate')
            self.__bitrate = 18
